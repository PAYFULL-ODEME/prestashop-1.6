...
class PayfullPaymentModuleFrontController extends ModuleFrontController
{
    ...
    protected function _processPayment($data)
    {
        ...
        $api = PayfullApi::instance($this->module, $language['iso_code']);
        ...
        if(isset($response['html'])) {
            ...
            try {
                PayfullApi::processPayResponse($this->module, $response);
                return true;
            } catch (Exception $ex) {
    ...
    protected function _preparePaymentInfo($api, $data)
    {
        ...
        Db::getInstance()->insert("payfull_transaction", array(
            'cart_id' => (int) $cart->id,
            'card_holder' => $payment['cc_name'],
            'crard_mask' => PayfullApi::getCardMask($payment['cc_number']),
        ...
        //$this->module->validateOrder($cart->id, Configuration::get('PS_OS_PAYFULL_PENDING'), $orderTotal, $this->module->name, NULL, [], (int)$currency['id'], false, $customer->secure_key);
        
        //$currency = $this->context->currency;
        $invoice = new Address((int)$cart->id_address_invoice);
        
        $payment['total'] = (float)Tools::ps_round($orderTotal, 2);
        ...
        $payment['transaction_id'] = $last_insert_id;
        $payment['payment_title'] = strtr('Payfull Payment: {firstName} {lastName}. Total {total} {currency}', [
        ...
        if($payment['use3d'] === 1) {
            ...
            $new_cart_id=$this->_placeOrder(Configuration::get('PS_OS_PAYFULL_PENDING'), $orderTotal, $message, ['transaction_id'=>$payment['transaction_id']],$payment['use3d']);
            $returnUrl = $this->context->link->getModuleLink('payfull', 'process3D', [], true);
    ...
    protected function _placeOrder($status, $total, $message, $extra=[],$use3D=0)
    {
        $cart = $this->context->cart;
        $cart_id=$cart->id;
        $customer = new Customer($cart->id_customer);
    ...
    protected function _redirectToConfirmation()
    {
        ...
        $orderId = $this->module->currentOrder;
        $returnUrl = $this->context->link->getModuleLink('payfull', 'confirmation', ['order_id'=>$orderId, 'secure_key'=>$key], true);
